...
<div class="col-md-12">
    ...
    <a href="#" id = "addButton" data-bs-target="#addModal" data-bs-toggle="modal" class = "btn btn-primary btn-info col-md-2">Add</a>
        <table id="table-checkout" class = "table" style = "margin-top: 10px">
            ...
            <tbody>
            <input type="text" class="form-control" value="{{ $order->id_order }}" name="id_order" id="id_order" hidden>
            @if(isset($detail))
              @foreach( $detail as $row )
            <tr>
              <td width = "3%"><a href="#" id = "deleteRow" data-harga ="{{$row->subtotal}}"><img src="../symbols/trash.png" style="width:20px;height:20px;"></a></td>
              <td>{{ $row->menu_id_menu }}<input type="text" class="form-control" value="{{ $row->menu_id_menu }}" name="nama_menu[]" hidden></td>
              <td>{{ $row->Varian }}<input type="text" class="form-control" value="{{ $row->Varian }}" name="varian[]" hidden></td>
              <td>{{ $row->jumlah }}<input type="text" class="form-control" value="{{ $row->jumlah }}" name="qty[]" hidden></td>
              <td>{{ $row->subtotal }}<input type="text" class="form-control" value="{{ $row->subtotal }}" name="subtotal[]" hidden></td>
            </tr>
              @endforeach
            @endif
            </tbody>
        ...
        <div id="total_harga" class="total_harga" style="text-align: right; margin-top:50px;">
            ...
            <input type="text" class="" id="total" name="total" value="{{ $order->total }}" readonly>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
            ...
            <label for="" class="col-md-2" style ="color:black;">Jenis Pembayaran</label>
                <div class="col-md-5 offset-md-4">
                    <select name="jenis_pembayaran" id="jenis_pembayaran" class="form-control" onchange = "bayarToggle()">
                        ...
                        <option value="Ovo" hidden>Ovo</option>
                        <option value="Grab" hidden>Grab</option>
                        <option value="ShopeePay" hidden>ShopeePay</option>
        ...
        <div id="toggleTransfer" style="display:none">
          <button type="button" class="btn btn-outline-secondary btnToggle active">Ovo</button>
          <button type="button" class="btn btn-outline-secondary btnToggle">Grab</button>
          <button type="button" class="btn btn-outline-secondary btnToggle">ShopeePay</button>
          <button type="button" class="btn btn-outline-secondary btnToggle">QRIS</button>
        </div>
...
var header = document.getElementById("toggleTransfer");
var btns = header.getElementsByClassName("btnToggle");
for (var i = 0; i < btns.length; i++) {
  btns[i].addEventListener("click", function() {
  var current = document.getElementsByClassName("active");
  current[0].className = current[0].className.replace(" active", "");
  this.className += " active";
  $("#jenis_pembayaran").val(this.innerText).change();
